...
## The MVP: cohort-builder user stories

The MVP is defined by the **user stories** in
[`resources/sysbio-cdm-user-stories.md`](resources/sysbio-cdm-user-stories.md) — the
questions a cohort builder must be able to answer of the CDM. They are the acceptance
criteria, and they are checked automatically on every build:

```sh
make verify-stories     # runs the story queries against the built CDM (also part of `make all`)
```

`scripts/14_verify_user_stories.py` runs each story's query against the built CDM and
asserts it returns data — the build fails if any regresses. Runnable, documented versions
of every query live in
[`resources/query_cookbook.md`](resources/query_cookbook.md). It is **14 checks** (the 13
stories plus a cross-silo cohort query); **14/14 passing = MVP threshold met.**
...
`specs/table_schema_fields.tsv` and `specs/ark_fields.tsv` are regenerated on every run at a
**fixed filename** — the spec is deterministic, so they are overwritten byte-identically (a
real dictionary change shows as a clean git diff; nothing accumulates). The write-only
`specs/table_schemas/` directory is gitignored.
...
a meta-test that a deliberately broken cohort fails QC. Determinism/golden only — for the
**MVP user-story acceptance gate** run `make verify-stories` (see [the MVP section](#the-mvp-cohort-builder-user-stories) at the top), which checks the story queries against a *loaded* CDM.
...
- No round-trip verification. The cell-for-cell verifier (`07_verify_cdm.py`) reconstructed
  every value from the CDM and diffed it against the source, but derived its expected cells
  by parsing the previous *external* ETL — so it was removed when that integration was. Rework
  to reconstruct expected cells from the map-driven load (`mappings/*.json` + `09_map_etl.py`)
  is outstanding.
- `04_qc.py`, `13_verify_governance.py`, and `14_verify_user_stories.py` are the pipeline's
  own checks — they validate internal consistency, access control, and user-story
  answerability, not clinical validity or cell-for-cell ETL fidelity (see round-trip above).
